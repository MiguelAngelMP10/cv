# Miguel Ángel Muñoz Pozos's CV

- Phone: +52 241 112 5657
- Email: [mmunozpozos@gmail.com](mailto:mmunozpozos@gmail.com)
- Location: México
- LinkedIn: [MiguelAngelMP10](https://linkedin.com/in/MiguelAngelMP10)
- GitHub: [MiguelAngelMP10](https://github.com/MiguelAngelMP10)


# Perfil Profesional
Desarrollador Full Stack especializado en backend PHP y comercio electrónico. Experiencia en soluciones web escalables y automatización de procesos fiscales y comerciales, con enfoque en rendimiento, calidad de código y buenas prácticas.

# Experiencia Profesional
## **Instituto Tlaxcalteca de Elecciones (ITE)**, Titular del Área Técnica de Informática (ATI)

Tlaxcala, México

Ago 2026 – presente

- Coordinación y supervisión de los sistemas informáticos institucionales para procesos electorales.

- Gestión de infraestructura tecnológica, seguridad de la información y continuidad operativa.

- Liderazgo de equipos técnicos y toma de decisiones críticas en entornos de alta disponibilidad.

- Desarrollo, mantenimiento e integración de sistemas internos para registro, validación y control de información.

- Implementación de buenas prácticas de desarrollo, control de accesos y auditoría de sistemas.



## **Instituto Tlaxcalteca de Elecciones (ITE)**, Jefe Provisional del Área Técnica de Informática (ATI)

Tlaxcala, México

Nov 2025 – Ago 2026

- Coordinación y supervisión de los sistemas informáticos institucionales para procesos electorales.

- Gestión de infraestructura tecnológica, seguridad de la información y continuidad operativa.

- Liderazgo de equipos técnicos y toma de decisiones críticas en entornos de alta disponibilidad.

- Desarrollo, mantenimiento e integración de sistemas internos para registro, validación y control de información.

- Implementación de buenas prácticas de desarrollo, control de accesos y auditoría de sistemas.



## **Crimson Circle**, Desarrollador Full Stack (Magento 2)

Remoto

Abr 2025 – Ago 2025

- Desarrollo de módulo en Magento 2 para precálculo de precios basado en reglas de cliente.

- Optimización de rendimiento y experiencia de compra (mejoras en tiempos de carga).

- Aplicación del SDLC y principios de organizaciones de alta fiabilidad (HRO).



## **Softtek**, Ingeniero de Software

Remoto

Oct 2024 – Feb 2025

- Revisión y evaluación de código PHP bajo estándares de calidad y seguridad.

- Capacitación técnica continua y fortalecimiento de buenas prácticas.



## **Freelance**, Desarrollador Full Stack

Remoto

Abr 2024 – Oct 2024

- Desarrollo de sistemas de automatización fiscal y comercial.

- Implementación de descarga masiva de CFDI, POS y control de deudas.

- Desarrollo de APIs REST y scrapers especializados del SAT.



## **BESTA Arrendadora**, Desarrollador Full Stack Middle

Remoto

Dic 2023 – Abr 2024

- Desarrollo y mantenimiento de módulos administrativos.

- Generación de reportes y consultas internas.

- Soporte a clientes internos.



## **Wagento Commerce**, Desarrollador JR Magento 2

Remoto

Oct 2021 – Sep 2023

- Desarrollo y configuración de tiendas Magento 2.

- Implementación de pasarelas de pago y módulos personalizados.



## **Enlace Athenea S.A. de C.V.**, Tester de Software

Puebla, México

Sep 2020 – Oct 2021

- Pruebas funcionales, de integración y rendimiento de aplicaciones educativas.



## **Tecnológico Nacional de México – Campus Apizaco**, Desarrollador Web

Apizaco, Tlaxcala

Feb 2021 – Mar 2023

- Desarrollo de módulos para sistemas de información.

- Creación de APIs REST utilizando Laravel.



# Formación Académica
## **Tecnológico Nacional de México – Campus Apizaco**, Maestría en Sistemas Computacionales



Apizaco, Tlaxcala

Ago 2018 – Jun 2021



## **Tecnológico Nacional de México – Campus Apizaco**, Licenciatura en Tecnologías de la Información y Comunicaciones



Apizaco, Tlaxcala

Ago 2013 – Jun 2018



# Sector de Experiencia
**Áreas:** Comercio electrónico · Desarrollo web · Bases de datos · Desarrollo de software

# Especialización
**Enfoque:** Backend PHP · Comercio electrónico con Magento 2

# Habilidades Técnicas
**Lenguajes Backend:** PHP · Node.js · Python · Java · TypeScript

**Lenguajes Frontend:** JavaScript · React · Next.js · Vue.js · Angular · Svelte

**Frameworks:** Laravel · Yii2

**Bases de datos:** MySQL · MariaDB · PostgreSQL · SQLite · MongoDB

**Comercio electrónico:** Magento 2 · WooCommerce · VTEX

**DevOps y control de versiones:** Docker · Git · GitHub · Bitbucket · GitKraken

**Herramientas de desarrollo:** Composer · NPM · Postman · Insomnia · Chrome DevTools

# Certificaciones
**Backend y Automatización (Platzi · 2025):** Prompt Engineering · Automatización con n8n · Backend PHP Avanzado

**Desarrollo Backend (Platzi · 2025):** Node.js · TypeScript · Java · Spring Framework

# Aptitudes
**Habilidades blandas:** Resolución de problemas · Trabajo en equipo · Adaptabilidad · Organización · Formación continua
